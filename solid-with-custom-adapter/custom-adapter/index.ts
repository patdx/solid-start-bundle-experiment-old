...
export default function ({ durableObjects = [] } = {}) {
  return {
    ...
    async build(config, builder) {
      const solidJsWebPath = fileURLToPath(
        await resolve("solid-js/web", import.meta.url)
      );
      console.log(solidJsWebPath);
      ...
      const bundle = await rollup({
        ...
        plugins: [
          ...
          nodeResolve({
            ...
            // only resolve the rollup to resolve solid-js ina "node-like" way
            resolveOnly: ["solid-js"],
          }),
          common(),
        ],
